	...
	for (;;) {
		...
		case KVM_EXIT_IO:
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN
				...
				char *p = (char *)vcpu->kvm_run;
				p[vcpu->kvm_run->io.data_offset] = exitcnt;
				continue;
			}

			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
			    && vcpu->kvm_run->io.port == 0xEC) {
				struct kvm_translation kvmtrl;
				char *p = (char *)vcpu->kvm_run;
				uint32_t gvmem;
				memcpy(&gvmem, p + vcpu->kvm_run->io.data_offset, vcpu->kvm_run->io.size);
				kvmtrl.linear_address = gvmem;
				if (ioctl(vcpu->fd, KVM_TRANSLATE, &kvmtrl) < 0) {
						perror("failed to translate gva to gpa");
						exit(1);
				}
				if(kvmtrl.valid ==1 ){
					printf("%s",vm->mem + kvmtrl.physical_address);
				}
				continue;
			}
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
				...
				if(kvmtrl.valid ==1 ){
					phyaddr = (uintptr_t  )(vm->mem + kvmtrl.physical_address);	
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN
				...
				char *p = (char *)vcpu->kvm_run;
				if(flag !=0){
				*(uint32_t *) (p + vcpu->kvm_run->io.data_offset) = phyaddr;}
				else{
					*(uint32_t *) (p + vcpu->kvm_run->io.data_offset) = 0;
				}
				fflush(stdout);
				continue;
			}
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN
			    && vcpu->kvm_run->io.port == 0xEF) {
				char *p = (char *)vcpu->kvm_run;
				
				struct kvm_clock_data clk;
				if (ioctl(vm->fd, KVM_GET_CLOCK, &clk) < 0) {
					perror("KVM_GET_CLOCK");
					exit(1);
				}
				*(uint32_t *) (p + vcpu->kvm_run->io.data_offset) =(uint32_t) clk.clock;
				fflush(stdout);
				continue;
			}
